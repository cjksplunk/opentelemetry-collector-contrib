		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample":
					...
					attrVal, ok = lr.Attributes().Get("postgresql.procedure_id")
					assert.True(t, ok)
					assert.Equal(t, "postgresql.procedure_id-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("postgresql.procedure_name")
					assert.True(t, ok)
					assert.Equal(t, "postgresql.procedure_name-val", attrVal.Str())
