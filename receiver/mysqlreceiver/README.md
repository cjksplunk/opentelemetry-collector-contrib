...
- `resource_attributes_ovveride`: (optional) A map of resource attributes to override or add to a receiver instance. This is useful when you have multiple instances of the same receiver and want to differentiate them by adding custom resource attributes. For example, you can add an attribute `instance: mysql-1` to one instance and `instance: mysql-2` to another instance. This implementation uses the same logic as resource and attributres processors in an UPSERT mode.
